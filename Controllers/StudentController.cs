...
namespace BDolma_SMS.Controllers
{
    ...
    {
        private readonly ICrudService<Student> _studentInfoService;
        private readonly ICrudService<Course> _courseInfoService;
        private readonly UserManager<ApplicationUser> _userManager;

        public StudentController(ICrudService<Student> studentInfoService,
            ICrudService<Course> courseInfoService,
            ...
            ViewBag.Sections = new List<SelectListItem>
     {
        ...
        [Authorize(Roles = "ADMIN")]
        [HttpPost]
        public async Task<IActionResult> AddEdit(Student studentInfo)
        {
            var userId = _userManager.GetUserId(HttpContext.User);
            var user = await _userManager.FindByIdAsync(userId);
